...
class B(_InlineKeyboardButton):
    """Inline-кнопка с поддержкой Telegram-стилей (style/icon_custom_emoji_id)."""
    ...
    def __init__(self, text, url=None, callback_data=None, web_app=None, switch_inline_query=None,
        ...
        super().__init__(text, url=url, callback_data=callback_data, web_app=web_app,
...
class VPNStorage:
    ...
    def backup_to(self, dest: Path) -> None:
        """Создаёт консистентный SQLite-бэкап в указанный файл."""
        with self._lock:
            backup_conn = sqlite3.connect(str(dest), check_same_thread=False)
            try:
                with sqlite3.connect(str(DB_FILE), check_same_thread=False) as src:
    ...
    def add_pending_payment(self, payment_id: str, user_id: int, rub_amount: Decimal, method: str, status: str = "pending") -> None:
        with self._lock:
            self._db_conn.execute(
                "INSERT OR REPLACE INTO pending_payments (payment_id, user_id, rub_amount, method, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (payment_id, int(user_id), str(rub_amount), method, status, time.time()),
            )
            self._db_conn.commit()

    def get_pending_payment(self, payment_id: str) -> dict[str, Any] | None:
        with self._lock:
            row = self._db_conn.execute(
                "SELECT payment_id, user_id, rub_amount, method, status, created_at FROM pending_payments WHERE payment_id = ?",
                (payment_id,),
            ).fetchone()
        if not row:
            return None
        return {
            "payment_id": row[0],
            "user_id": row[1],
            "rub_amount": _to_dec(row[2]),
            "method": row[3],
            "status": row[4],
            "created_at": row[5],
        }

    def pending_payments(self, method: str | None = None) -> list[str]:
        with self._lock:
            if method:
                rows = self._db_conn.execute(
                    "SELECT payment_id FROM pending_payments WHERE method = ? AND status = 'pending'",
                    (method,),
                ).fetchall()
            else:
                rows = self._db_conn.execute(
                    "SELECT payment_id FROM pending_payments WHERE status = 'pending'",
                ).fetchall()
        return [r[0] for r in rows]

    def mark_pending_payment(self, payment_id: str, status: str) -> None:
        with self._lock:
            self._db_conn.execute(
    ...
    def unbind_device(self, sub_id: str, index: int, user_id: int) -> tuple[bool, str]:
        ...
        removed_dev = sub.devices.pop(index)
        user["last_unbind_at"] = now
        self.update_subscription(sub)
        self.update_user(user)
        if removed_dev.get("client_email"):
            try:
                XrayAPI.remove_client_by_email(removed_dev["client_email"])
            except Exception:
                pass
...
class MetricsCollector:
    ...
    def collect() -> dict[str, Any]:
        ...
        txs = list(storage.transactions.get("txs", {}).values())

        total_users = len(users)
        new_users_24h = sum(1 for u in users if u.get("joined_at", 0) > day_ago)
        total_balance = sum((_to_dec(u.get("balance", 0)) for u in users), Decimal("0"))
        total_referral_balance = sum((_to_dec(u.get("referral_balance", 0)) for u in users), Decimal("0"))

        active_subs = [s for s in subs if s.get("active") and not storage._sub_from_dict(s).is_expired]
        expired_subs = [s for s in subs if not s.get("active") or storage._sub_from_dict(s).is_expired]
        trial_subs = [s for s in subs if s.get("plan_id") == "trial"]
        expired_30d = [s for s in expired_subs if storage._sub_from_dict(s).expires_at > month_ago]
        ...
        for s in active_subs:
            ...
            for d in sub.devices:
                if d.get("last_seen", 0) > five_min_ago:
                    online_now += 1
                total_traffic += int(d.get("traffic", 0) or 0)
            price = storage.price(sub.plan_id, 1)
            if price is not None and price > 0:
                mrr += price
            else:
                for dur in DURATIONS:
                    p = storage.price(sub.plan_id, dur)
                    if p is not None and dur:
                        mrr += _money_round(p / Decimal(dur))
                        break

        revenue = Decimal("0")
        for tx in txs:
            amount = _to_dec(tx.get("amount", 0))
            if tx.get("type") == "purchase" and amount < 0:
                revenue += -amount

        total_subs = len(subs) or 1
        churn_rate = round(len(expired_30d) / total_subs, 4) if subs else 0.0
        ...
        return {
            ...
            "subscriptions_trial": len(trial_subs),
            "subscriptions_expired_30d": len(expired_30d),
            "online_now": online_now,
            "total_traffic_bytes": total_traffic,
            "total_balance_rub": float(_money_round(total_balance)),
            "total_referral_balance_rub": float(_money_round(total_referral_balance)),
            "mrr_rub": float(_money_round(mrr)),
            "revenue_total_rub": float(_money_round(revenue)),
            "churn_rate_30d": churn_rate,
            "timestamp": now,
        }

    @staticmethod
    def to_prometheus(data: dict[str, Any]) -> str:
        lines = ["# HELP vpn_metrics Generated by VPN bot plugin", "# TYPE vpn_metrics gauge"]
...
class BackupScheduler:
    """Фоновый поток бэкапов SQLite с отправкой админу."""
    ...
    def _run(self) -> None:
        # первый бэкап через час после старта, чтобы не грузить загрузку
        ...
        while not self._stop.is_set():
            try:
                ...
                logger.exception("BackupScheduler error")
            interval = max(1, int(storage.config.get("backup_interval_hours", 24))) * 3600
            if self._stop.wait(interval):
                return

    def _backup(self) -> None:
        if not storage.config.get("backup_enabled", True):
            return
        backups_dir = STORAGE_DIR / "backups"
        backups_dir.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        raw = backups_dir / f"vpn_bot_{stamp}.db"
        gz = raw.with_suffix(".db.gz")
        try:
            storage.backup_to(raw)
            with open(raw, "rb") as f_in, gzip.open(gz, "wb") as f_out:
                shutil.copyfileobj(f_in, f_out)
            logger.info("SQLite backup created: %s", gz)
        finally:
            if raw.exists():
                raw.unlink()

        if storage.config.get("backup_send_admin", True):
            self._send_to_admins(gz, stamp)

        self._cleanup_old(backups_dir)

    def _send_to_admins(self, gz: Path, stamp: str) -> None:
        bot = getattr(self.bot, "bot", None)
        if not bot:
            bot = getattr(_user_bot_instance, "bot", None)
        if not bot:
            return
        recipients = set()
        for uid in storage.config.get("admin_user_ids", []):
            try:
                recipients.add(int(uid))
            except (ValueError, TypeError):
                pass
        ...
        if support_id:
            try:
                ...
                pass
        caption = f"Автобэкап VPN-бота {stamp}"
        for uid in recipients:
            try:
                with open(gz, "rb") as f:
                    bot.send_document(uid, f, caption=caption)
            except Exception:
                logger.exception("Backup send to admin %s failed", uid)

    def _cleanup_old(self, backups_dir: Path) -> None:
        keep = max(1, int(storage.config.get("backup_keep_count", 7)))
        files = sorted(backups_dir.glob("vpn_bot_*.db.gz"), key=lambda p: p.stat().st_mtime, reverse=True)
...
class DeviceAuthServer:
    ...
    def _build_app(self) -> Any:
        ...
        async def metrics(request: Request) -> Any:
            ...
            return Response(MetricsCollector.to_prometheus(data), media_type="text/plain; version=0.0.4; charset=utf-8")

        @app.get("/metrics.json")
        async def metrics_json(request: Request) -> Any:
            ip = request.client.host if request.client else "unknown"
            if not DeviceAuthServer._rate_limit(ip, max_req=120, window=60):
                raise HTTPException(status_code=429)
        ...
        async def subscription(sub_id: str, request: Request, format: str = "json") -> Any:
            ...
            vless_url = generate_vless_url(sub, server, client_uuid=client_uuid)
            if fmt == "raw":
            ...
            if fmt == "json":
                return JSONResponse(build_v2raytun_json(vless_url, server, sub, client_uuid=client_uuid), headers=headers)
            if fmt == "singbox":
                return JSONResponse(build_singbox_config(vless_url, server, sub, client_uuid=client_uuid), headers=headers)
            if fmt == "clash":
                return Response(content=build_clash_config(vless_url, server, sub, client_uuid=client_uuid), headers=headers, media_type="text/yaml; charset=utf-8")
...
class XrayAPI:
    ...
    def remove_client(sub: Subscription) -> bool:
        ...
        for email in {sub.email} | {d.get("client_email") for d in sub.devices}:
            if email and XrayAPI.remove_client_by_email(email, server.inbound_id, sub.host_id):
                removed = True
        if removed:
            sub.xray_synced = False
            storage.update_subscription(sub)
            logger.info("3X-UI clients removed for sub #%s host=%s", sub.sub_id, sub.host_id)
            return True
        return False
    ...
    @staticmethod
    def add_or_update_device_client(sub: Subscription, device: dict[str, Any], enable: bool = True) -> bool:
        """Добавляет/обновляет клиента в 3X-UI под конкретное устройство."""
        server = XrayAPI._get_host(sub.host_id)
        if not server or not server.panel_url or not sub.email:
            return False
    ...
    def get_client_stats(sub: Subscription) -> dict[str, int]:
        ...
        try:
            total_up = 0
            total_down = 0
            allowed_emails = {sub.email} | {d.get("client_email") for d in sub.devices}
            for stat in inbound.get("clientStats", []) or []:
                if stat.get("email") in allowed_emails or stat.get("subId") == sub.xray_sub_id:
                    total_up += int(stat.get("up", 0))
                    total_down += int(stat.get("down", 0))
...
def generate_vless_url(sub: Subscription, server: ServerConfig, client_uuid: str | None = None) -> str:
    """Собирает vless:// ссылку по данным подписки и сервера."""
    return generate_vless_url_raw(
        client_uuid=client_uuid or sub.client_uuid,
        email=sub.email,
...
def build_singbox_config(vless_url: str, server: ServerConfig, sub: Subscription, client_uuid: str | None = None) -> dict[str, Any]:
...
def build_clash_config(vless_url: str, server: ServerConfig, sub: Subscription, client_uuid: str | None = None) -> str:
...
def build_v2raytun_json(vless_url: str, server: ServerConfig, sub: Subscription, client_uuid: str | None = None) -> dict[str, Any]:
...
def _set_user_bot_menu_button(enabled: bool) -> None:
    ...
    try:
        ...
        if enabled:
            ...
            if not url:
                return
            menu = {"type": "web_app", "text": "Кабинет", "web_app": {"url": url}}
        else:
            menu = {"type": "commands"}
        r = requests.post(
...
def _handle_admin_callback(cardinal: Cardinal, c: CallbackQuery) -> None:
    ...
    if section == "backup" and len(args) <= 1:
        enabled = storage.config.get("backup_enabled", True)
        interval = storage.config.get("backup_interval_hours", 24)
        keep = storage.config.get("backup_keep_count", 7)
        send = storage.config.get("backup_send_admin", True)
        backups_dir = STORAGE_DIR / "backups"
        files = sorted(backups_dir.glob("vpn_bot_*.db.gz"), key=lambda p: p.stat().st_mtime, reverse=True) if backups_dir.exists() else []
        latest = files[0].name if files else "нет"
        text = (f"<b>💾 Бэкапы SQLite</b>\n\n"
                f"Автобэкап: {'Вкл' if enabled else 'Выкл'}\n"
                f"Интервал: {interval} ч\n"
                f"Хранить: {keep} шт\n"
                f"Отправлять админу: {'Да' if send else 'Нет'}\n"
                f"Последний: {latest}\n\n"
                f"Кнопки: включить/выключить, сделать бэкап сейчас.")
        kb = K()
        kb.add(B("Включить" if not enabled else "Выключить", callback_data=f"{CB_PREFIX}admin:backup:toggle"))
        kb.add(B("Бэкап сейчас", callback_data=f"{CB_PREFIX}admin:backup:now"))
        kb.add(B("Назад", callback_data=f"{CB_PREFIX}admin:main"))
        bot.edit_message_text(text, chat_id, c.message.message_id, reply_markup=kb)
        return

    if section == "backup" and len(args) >= 2:
        action = args[1]
        if action == "toggle":
            storage.config["backup_enabled"] = not storage.config.get("backup_enabled", True)
            storage.save_config()
            status = "включён" if storage.config["backup_enabled"] else "выключён"
            bot.answer_callback_query(c.id, f"Автобэкап {status}.")
            bot.edit_message_text(f"Автобэкап {status}.", chat_id, c.message.message_id,
                                  reply_markup=K().add(B("Назад", callback_data=f"{CB_PREFIX}admin:backup")))
        elif action == "now":
            if _user_bot_instance and _user_bot_instance._backup_scheduler:
                threading.Thread(target=_user_bot_instance._backup_scheduler._backup, daemon=True).start()
            bot.answer_callback_query(c.id, "Бэкап запущен, файл придёт админу.")
        return
